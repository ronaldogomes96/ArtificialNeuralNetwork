...
class MLPRegression:
    ...
    def predict(self, X_test):
        _, output_layer_output = self.feed_forward_propagation(X_test)

        return output_layer_output

    def fit(self, X, y):
        self.weights.append(np.random.uniform(0, 1, (X.shape[1], self.hidden_layer_neurons)))
        self.weights.append(np.random.uniform(0, 1, (self.hidden_layer_neurons, y.shape[1])))
    ...
    def feed_forward_propagation(self, X):
        if self.activation_function == 'hyperbolic_tangent':
            ...
            output_layer_output = np.dot(hidden_layer_output, self.weights[1])
    ...
    def back_propagation(self, X, y, hidden_layer_output, output_layer_output):
        ...
        hidden_layer_error = output_error.dot(self.weights[1].T)
        ...
        self.weights[1] += hidden_layer_output.T.dot(output_error) * self.learning_rate
